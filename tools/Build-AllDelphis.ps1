...
# ------------------------------------------------------------------------
# Generate packages\ZipFileORM.ProjectRoot.inc with the absolute root.
# This embeds the project location into the design-time BPL so that
# ZipFileORM.LibraryPathReg.pas can register Library Paths when the
# package is loaded by the IDE (Install Packages / startup).
# ------------------------------------------------------------------------
$incFile = Join-Path $pkgsDir 'ZipFileORM.ProjectRoot.inc'
$rootEscaped = $root.Replace("'", "''")
$incContent = @"
// ZipFileORM.ProjectRoot.inc
//
// AUTO-GENERATED by tools/Build-AllDelphis.ps1 before each build.
// DO NOT EDIT MANUALLY - changes here will be overwritten.
//
// Contains the absolute project root path embedded into the design-time
// BPL so that ZipFileORM.LibraryPathReg.pas can register the correct
// Library Paths when the package is loaded.

const
  cZipFileORMProjectRoot = '$rootEscaped';
"@
Set-Content -Path $incFile -Value $incContent -Encoding ASCII -NoNewline
Write-Host "Generated $incFile" -ForegroundColor DarkCyan
Write-Host "  cZipFileORMProjectRoot = '$root'" -ForegroundColor DarkCyan
Write-Host ""
