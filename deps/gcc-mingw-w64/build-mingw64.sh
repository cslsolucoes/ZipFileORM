...
# Date: 2026-05-10
# Notes:
# Built on Linux. Tested with Ubuntu 24.04.4 (WSL2).
# Installed build dependencies:
# sudo apt install -y unzip p7zip-full autoconf automake libtool build-essential bison flex texinfo texlive texlive-plain-generic cmake licensecheck help2man gengetopt osslsigncode opensc pcscd
...
PREFIX='/mingw64-64'  # target directory (ensure that this is writable as current user)
...
# packet versions
ZLIB='zlib-1.3.2'                       # https://zlib.net/
...
BINUTILS='binutils-2.46.0'              # https://ftp.gnu.org/gnu/binutils/
MINGW='mingw-w64-v14.0.0'               # https://sourceforge.net/projects/mingw-w64/
GCC='gcc-16.1.0'                        # https://gcc.gnu.org/
EXPAT='expat-2.8.0'                     # https://libexpat.github.io/
NCURSES='ncurses-6.6'                   # https://ftp.gnu.org/gnu/ncurses/
BOOST='boost_1_91_0'                    # https://www.boost.org/
HIGHLIGHT='source-highlight-3.1.9'      # https://ftp.gnu.org/gnu/src-highlite/
IPT='libipt-2.2'                        # https://github.com/intel/libipt
...
ICONV='libiconv-1.19'                   # https://www.gnu.org/software/libiconv/
PYTHON='Python-3.13.13'                 # https://www.python.org/downloads/source/
GDB='gdb-gdb-17.2.90.20260510'          # https://github.com/ssbssa/gdb
...
OPENSSL='openssl-3.4.5'                 # https://openssl-library.org/source/
P11='libp11-0.4.18'                     # https://github.com/OpenSC/libp11/
SC_HSM_EMBED='sc-hsm-embedded-2.12'     # https://github.com/CardContact/sc-hsm-embedded/
OPENPACE='openpace-1.1.4'               # https://github.com/frankmorgner/openpace/releases
OPENSC='OpenSC-0.27.1'                  # https://github.com/OpenSC/OpenSC/ (without manufacturer specific tools)
OSSLSIGNCODE='osslsigncode-2.13'        # https://github.com/mtrojnar/osslsigncode/releases
#SIGN=''                                 # path to script which accepts a single file as argument for code signing

# build configuration
GCC_LANGS='c,c++,lto'
GCC_HOST_CONFIG='--enable-seh-exceptions --with-arch=core2 --with-tune=generic --enable-threads=posix --disable-nls --disable-libstdcxx-verbose --disable-libstdcxx-pch --enable-clocale=generic --enable-shared=libstdc++ --enable-static --enable-libatomic --enable-fully-dynamic-string --enable-lto --enable-tls --enable-plugins --enable-libgomp --with-dwarf2 --disable-win32-registry --enable-version-specific-runtime-libs --enable-checking=release'
GCC_TARGET_CONFIG='--enable-seh-exceptions --with-arch=core2 --with-tune=generic --enable-threads=posix --disable-nls --disable-libstdcxx-verbose --disable-libstdcxx-pch --enable-clocale=generic --enable-shared=libstdc++ --enable-static --enable-libatomic --enable-fully-dynamic-string --enable-lto --enable-tls --enable-plugins --enable-libgomp --with-dwarf2 --enable-mingw-wildcard=platform --disable-win32-registry --enable-version-specific-runtime-libs --enable-checking=release'
...
if [ "$?" -eq 0 -o -d "/run/WSL" ]; then
	echo 0 | sudo tee '/proc/sys/fs/binfmt_misc/WSLInterop' >/dev/null 2>&1
fi
...
d75a94f4d73e7a4086f7513e67e439e8fcdcbb726ffe63f4661744e6256b2cf2  binutils-2.46.0.tar.xz
de5e6b0e4913395c6bdfa90537febd9028ea4c0735d2cdb0cd9b45d5f51264f5  boost_1_91_0.tar.bz2
d370cf9990d2be24bfb24750e355bac26110051248cabf2add61f9b3867fb1d7  cloog-0.21.1.tar.gz
a37bfae0aa9775bd8521ebd85dc456d486f0ff31138f6c91fd902ea732624542  expat-2.8.0.tar.xz
...
737f7e50d85d29e5a0e4a6487130955512b7fc88eef629685f40b6c252ed6c59  gdb-gdb-17.2.90.20260510.tar.gz
...
f09a18fefba81d4fc2530d90858789e0c596f1b634e5777e6ccaf492966e9845  libipt-2.2.tar.gz
9292de67ca73aba1deacf577c9086b595765f36ef47712cfeb49fa31f6e772fb  libp11-0.4.18.tar.gz
2f1eb3c15cad0e64e0dc4348cf560165e2d6918b7ef68f22b9202838765ebfea  mingw-w64-v14.0.0.zip
...
355b4cbbed880b0381a04c46617b7656e362585d52e9cf84a67e2009b749ff11  ncurses-6.6.tar.gz
...
36954f0385a1cb9805a02cffbd0d99f2b3a0598a125edfa160b4cca65b417854  openpace-1.1.4.tar.gz
9b72f1b5d92569de67a42bf0edb21ecdc685ad03378343416362a99b90b1fad1  OpenSC-0.27.1.tar.gz
c5a6929234db941eca709b79db71d3fbc9819ee91324cdeabf898436c20dcddc  openssl-3.4.5.tar.gz
ee95638b8bec0c019ddf28cb14988645abbd180dcd017536338b7d0d5eaaea96  osslsigncode-2.13.tar.gz
2ab91ff401783ccca64f75d10c882e957bdfd60e2bf5a72f8421793729b78a71  Python-3.13.13.tar.xz
...
if step 'patch GCC source'; then
	...
	# fix https://gcc.gnu.org/pipermail/gcc-patches/2025-May/683574.html
	sed -i 's|bool preserve_none_abi|int preserve_none_abi|g' 'config/i386/i386.h' || error "Failed to patch ${BUILD}/gcc-src/gcc/config/i386/i386.h."
...
if step "build ${NCURSES:-ncurses}"; then
	mkdir -p "${BUILD}/ncurses" || error "Failed to create ${BUILD}/ncurses."
	cd "${BUILD}/ncurses"
	"${SRC}/${NCURSES}/configure" --host=x86_64-w64-mingw32 --target=x86_64-w64-mingw32 --enable-static --disable-shared --disable-getcap --disable-hard-tabs --disable-home-terminfo --disable-lib-suffixes --disable-mixed-case --disable-overwrite --disable-rpath --disable-symlinks --disable-termcap --enable-assertions --enable-colorfgbg --enable-database --enable-exp-win32 --enable-ext-colors --enable-ext-funcs --enable-ext-mouse --enable-interop --enable-opaque-curses --enable-opaque-form --enable-opaque-menu --enable-opaque-panel --enable-sigwinch --enable-sp-funcs --enable-term-driver --enable-widec --with-cxx --with-fallbacks=ms-terminal --without-ada --without-debug --without-libtool --without-pthread --without-tests "--prefix=${HOST}/x86_64-w64-mingw32" >>"${LOG}" 2>&1 || error "Failed to configure ${NCURSES}."
...
diff -uarN Python-3.13.13-org/Include/internal/pycore_fileutils.h Python-3.13.13/Include/internal/pycore_fileutils.h
--- Python-3.13.13-org/Include/internal/pycore_fileutils.h	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Include/internal/pycore_fileutils.h	2026-04-25 17:32:20.941901100 +0200
@@ -12,7 +12,7 @@
...
+#if defined(_MSC_VER) || defined(__MINGW32__)
 ...
 #else
diff -uarN Python-3.13.13-org/Include/internal/pycore_time.h Python-3.13.13/Include/internal/pycore_time.h
--- Python-3.13.13-org/Include/internal/pycore_time.h	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Include/internal/pycore_time.h	2026-04-25 17:32:20.941901100 +0200
@@ -58,7 +58,7 @@
 #endif
...
diff -uarN Python-3.13.13-org/Include/pymacro.h Python-3.13.13/Include/pymacro.h
--- Python-3.13.13-org/Include/pymacro.h	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Include/pymacro.h	2026-04-25 17:32:20.957678400 +0200
@@ -98,7 +98,7 @@
...
diff -uarN Python-3.13.13-org/Modules/_blake2/impl/blake2b.c Python-3.13.13/Modules/_blake2/impl/blake2b.c
--- Python-3.13.13-org/Modules/_blake2/impl/blake2b.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/_blake2/impl/blake2b.c	2026-04-25 17:32:20.973712600 +0200
@@ -20,7 +20,7 @@
...
diff -uarN Python-3.13.13-org/Modules/_blake2/impl/blake2s.c Python-3.13.13/Modules/_blake2/impl/blake2s.c
--- Python-3.13.13-org/Modules/_blake2/impl/blake2s.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/_blake2/impl/blake2s.c	2026-04-25 17:32:20.973712600 +0200
@@ -20,7 +20,7 @@
...
diff -uarN Python-3.13.13-org/Modules/_ctypes/_ctypes_test.c Python-3.13.13/Modules/_ctypes/_ctypes_test.c
--- Python-3.13.13-org/Modules/_ctypes/_ctypes_test.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/_ctypes/_ctypes_test.c	2026-04-25 17:32:20.999291400 +0200
@@ -21,6 +21,11 @@
 #endif
...
+EXPORT(S8I) __stdcall DECORATE(s_ret_8i_func)(S8I inp) { return ret_8i_func(inp); }
 ...
 #ifdef MS_WIN32
diff -uarN Python-3.13.13-org/Modules/_ctypes/ctypes.h Python-3.13.13/Modules/_ctypes/ctypes.h
--- Python-3.13.13-org/Modules/_ctypes/ctypes.h	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/_ctypes/ctypes.h	2026-04-25 17:32:21.005574300 +0200
@@ -36,7 +36,7 @@
 #endif
...
+#include <unknwn.h> // for IUnknown interface
 ...
 typedef struct {
diff -uarN Python-3.13.13-org/Modules/_decimal/libmpdec/mpdecimal.h Python-3.13.13/Modules/_decimal/libmpdec/mpdecimal.h
--- Python-3.13.13-org/Modules/_decimal/libmpdec/mpdecimal.h	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/_decimal/libmpdec/mpdecimal.h	2026-04-25 17:32:21.005574300 +0200
@@ -30,7 +30,7 @@
...
diff -uarN Python-3.13.13-org/Modules/_ssl.c Python-3.13.13/Modules/_ssl.c
--- Python-3.13.13-org/Modules/_ssl.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/_ssl.c	2026-04-25 17:32:21.021307800 +0200
@@ -5885,7 +5885,7 @@
     return result;
 }
...
diff -uarN Python-3.13.13-org/Modules/_winapi.c Python-3.13.13/Modules/_winapi.c
--- Python-3.13.13-org/Modules/_winapi.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/_winapi.c	2026-04-25 17:32:21.037027900 +0200
@@ -2995,7 +2995,9 @@
...
diff -uarN Python-3.13.13-org/Modules/clinic/_ssl.c.h Python-3.13.13/Modules/clinic/_ssl.c.h
--- Python-3.13.13-org/Modules/clinic/_ssl.c.h	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/clinic/_ssl.c.h	2026-04-25 17:32:21.052716900 +0200
@@ -2706,7 +2706,7 @@
     return return_value;
 }
...
diff -uarN Python-3.13.13-org/Modules/faulthandler.c Python-3.13.13/Modules/faulthandler.c
--- Python-3.13.13-org/Modules/faulthandler.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/faulthandler.c	2026-04-25 17:32:21.068733100 +0200
@@ -999,7 +999,7 @@
     }
 #endif
...
diff -uarN Python-3.13.13-org/Modules/mathmodule.c Python-3.13.13/Modules/mathmodule.c
--- Python-3.13.13-org/Modules/mathmodule.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/mathmodule.c	2026-04-25 17:32:21.068733100 +0200
@@ -2404,7 +2404,7 @@
...
diff -uarN Python-3.13.13-org/Modules/posixmodule.c Python-3.13.13/Modules/posixmodule.c
--- Python-3.13.13-org/Modules/posixmodule.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/posixmodule.c	2026-04-25 17:32:21.084713900 +0200
@@ -43,6 +43,10 @@
...
diff -uarN Python-3.13.13-org/Modules/selectmodule.c Python-3.13.13/Modules/selectmodule.c
--- Python-3.13.13-org/Modules/selectmodule.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/selectmodule.c	2026-04-25 17:32:21.100507100 +0200
@@ -168,7 +168,7 @@
...
diff -uarN Python-3.13.13-org/Modules/socketmodule.c Python-3.13.13/Modules/socketmodule.c
--- Python-3.13.13-org/Modules/socketmodule.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/socketmodule.c	2026-04-25 17:32:21.116271200 +0200
@@ -279,8 +279,9 @@
 # endif
...
diff -uarN Python-3.13.13-org/Modules/timemodule.c Python-3.13.13/Modules/timemodule.c
--- Python-3.13.13-org/Modules/timemodule.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Modules/timemodule.c	2026-04-25 17:32:21.132037300 +0200
@@ -41,7 +41,7 @@
...
diff -uarN Python-3.13.13-org/Objects/dictobject.c Python-3.13.13/Objects/dictobject.c
--- Python-3.13.13-org/Objects/dictobject.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Objects/dictobject.c	2026-04-25 17:32:21.132037300 +0200
@@ -582,7 +582,7 @@
...
+#elif defined(_MSC_VER) || defined(__MINGW32__)
     ...
     minsize = Py_MAX(minsize, PyDict_MINSIZE);
     unsigned long msb;
diff -uarN Python-3.13.13-org/PC/_testconsole.c Python-3.13.13/PC/_testconsole.c
--- Python-3.13.13-org/PC/_testconsole.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/PC/_testconsole.c	2026-04-25 17:32:21.147935100 +0200
@@ -140,7 +140,7 @@
 }
...
diff -uarN Python-3.13.13-org/PC/_wmimodule.cpp Python-3.13.13/PC/_wmimodule.cpp
--- Python-3.13.13-org/PC/_wmimodule.cpp	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/PC/_wmimodule.cpp	2026-04-25 19:10:55.276977500 +0200
@@ -14,9 +14,9 @@
 #endif
...
diff -uarN Python-3.13.13-org/PC/pyconfig.h.in Python-3.13.13/PC/pyconfig.h.in
--- Python-3.13.13-org/PC/pyconfig.h.in	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/PC/pyconfig.h.in	2026-04-25 17:32:21.163625500 +0200
@@ -170,8 +170,8 @@
...
diff -uarN Python-3.13.13-org/PC/pylauncher.rc Python-3.13.13/PC/pylauncher.rc
--- Python-3.13.13-org/PC/pylauncher.rc	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/PC/pylauncher.rc	2026-04-25 17:32:21.179350200 +0200
@@ -12,17 +12,17 @@
...
diff -uarN Python-3.13.13-org/PC/pyshellext.cpp Python-3.13.13/PC/pyshellext.cpp
--- Python-3.13.13-org/PC/pyshellext.cpp	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/PC/pyshellext.cpp	2026-04-25 17:32:21.179350200 +0200
@@ -1,17 +1,13 @@
...
diff -uarN Python-3.13.13-org/PC/python_exe.rc Python-3.13.13/PC/python_exe.rc
--- Python-3.13.13-org/PC/python_exe.rc	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/PC/python_exe.rc	2026-04-25 17:32:21.199673200 +0200
@@ -12,7 +12,7 @@
...
+1 ICON DISCARDABLE "icons/python.ico" 
 ...
 /////////////////////////////////////////////////////////////////////////////
diff -uarN Python-3.13.13-org/PC/pythonw_exe.rc Python-3.13.13/PC/pythonw_exe.rc
--- Python-3.13.13-org/PC/pythonw_exe.rc	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/PC/pythonw_exe.rc	2026-04-25 17:32:21.199673200 +0200
@@ -12,7 +12,7 @@
 // current versions of Windows.
 1 RT_MANIFEST "python.manifest"
...
+1 ICON DISCARDABLE "icons/pythonw.ico" 
 ...
 /////////////////////////////////////////////////////////////////////////////
diff -uarN Python-3.13.13-org/PCbuild/Makefile Python-3.13.13/PCbuild/Makefile
--- Python-3.13.13-org/PCbuild/Makefile	1970-01-01 01:00:00.000000000 +0100
+++ Python-3.13.13/PCbuild/Makefile	2026-04-25 19:15:22.318025600 +0200
@@ -0,0 +1,662 @@
+# user configuration
+BASE = .
+DST = $(BASE)/mingw
...
+LIB_EXPAT = -lexpat
+PY_TARGETS += _elementtree pyexpat
+endif
+
...
+$(DST)/_wmi$(PY_PYD): $(addprefix $(DST)/PCbuild/,$(OBJ__wmi)) $(DST)/_wmi.rc.o |$(DST)/$(PY_DLL)
...
+$(DST)/%/:
+	mkdir -p $@
diff -uarN Python-3.13.13-org/Python/dynamic_annotations.c Python-3.13.13/Python/dynamic_annotations.c
--- Python-3.13.13-org/Python/dynamic_annotations.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Python/dynamic_annotations.c	2026-04-25 17:32:21.242477500 +0200
@@ -27,7 +27,7 @@
...
diff -uarN Python-3.13.13-org/Python/dynload_win.c Python-3.13.13/Python/dynload_win.c
--- Python-3.13.13-org/Python/dynload_win.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Python/dynload_win.c	2026-04-25 17:32:21.258246100 +0200
@@ -152,7 +152,7 @@
...
diff -uarN Python-3.13.13-org/Python/fileutils.c Python-3.13.13/Python/fileutils.c
--- Python-3.13.13-org/Python/fileutils.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Python/fileutils.c	2026-04-25 17:32:21.258246100 +0200
@@ -1185,7 +1185,11 @@
...
+#if defined(MS_WINDOWS) && Py_WINVER >= 0x0603
     if (FAILED(PathCchCombineEx(buffer, bufsize, dirname, relfile,
         ...
         return -1;
diff -uarN Python-3.13.13-org/Python/mysnprintf.c Python-3.13.13/Python/mysnprintf.c
--- Python-3.13.13-org/Python/mysnprintf.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Python/mysnprintf.c	2026-04-25 17:32:21.274328200 +0200
@@ -66,7 +66,7 @@
         goto Done;
     }
...
diff -uarN Python-3.13.13-org/Python/pylifecycle.c Python-3.13.13/Python/pylifecycle.c
--- Python-3.13.13-org/Python/pylifecycle.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Python/pylifecycle.c	2026-04-25 17:32:21.290144700 +0200
@@ -3573,7 +3573,7 @@
 #else
...
+#if (defined(_MSC_VER) && _MSC_VER >= 1400) || defined(__MINGW32__)
     ...
     case SIGINT:
diff -uarN Python-3.13.13-org/Python/pytime.c Python-3.13.13/Python/pytime.c
--- Python-3.13.13-org/Python/pytime.c	2026-04-07 20:19:01.000000000 +0200
+++ Python-3.13.13/Python/pytime.c	2026-04-25 17:32:21.299597900 +0200
@@ -893,6 +893,43 @@
 #endif
...
if step "build ${P11:-p11}"; then
	if [ "x${P11}" != 'x' ]; then
		...
		cd "${BUILD}/p11"
		"${SRC}/${P11}/configure" --host=x86_64-w64-mingw32 --disable-static --enable-shared --disable-dependency-tracking "--prefix=${HOST}/x86_64-w64-mingw32" "OPENSSL_CFLAGS=-I${HOST}/x86_64-w64-mingw32/include" "OPENSSL_LIBS=-L${HOST}/x86_64-w64-mingw32/lib -lcrypto" >>"${LOG}" 2>&1 || error "Failed to configure ${P11}."
...
if step "build ${OPENSC:-opensc}"; then
	if [ "x${OPENSC}" != 'x' ]; then
		...
		OPENSC_EXPORT='src/libopensc/libopensc.exports'
		[ -f 'src/libopensc/opensc.exports' ] && OPENSC_EXPORT='src/libopensc/opensc.exports'
		echo 'sc_aux_data_get_md_flags' >>"${OPENSC_EXPORT}" || error "Failed to patch ${OPENSC}/${OPENSC_EXPORT}."
		echo 'pcsc_check_reader_handles' >>"${OPENSC_EXPORT}" || error "Failed to patch ${OPENSC}/${OPENSC_EXPORT}."
		echo 'sc_pkcs1_strip_02_padding_constant_time' >>"${OPENSC_EXPORT}" || error "Failed to patch ${OPENSC}/${OPENSC_EXPORT}."
...
_EOF
		...
		install -m 644 'src/libopensc/.libs/libopensc-13-x64.dll' "${PREFIX}/bin/libopensc-13-x64.dll" || error "Failed to install ${OPENSC}/libopensc-13-x64.dll."
...
_EOFBAT
	...
	rm -rf "${BUILD}/result-to-zip" >/dev/null 2>&1
	cp -rL --link . "${BUILD}/result-to-zip/" || error "Failed to copy/dereference ${PREFIX} to ${BUILD}/result-to-zip/."
	cd "${BUILD}/result-to-zip/"
	7z a -t7z -mx9 -myx -md192m -mfb273 -ms=on "${ROOT}/${GCC}-${GDB}-${BINUTILS}-${MINGW}-${MCRTDLL}.7z" * || error "Failed to pack ${GCC}-${GDB}-${BINUTILS}-${MINGW}-${MCRTDLL}.7z."
